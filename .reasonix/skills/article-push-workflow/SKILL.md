...
description: 全自动推送管线：从选题池抽词→生成文章→审核修复→排版（两套版本），零人工干预
---
...
> ⚠️ **选题机制**：`draw_prompts.py` 从**选题池**中按**6个分组**（选科分数/适合人群/专业对比/学习技能/就业发展/填报策略）**不同分组各选1篇**，确保每天两篇主题差异大。标题自动做微调（加年份前缀、同义替换等）以避免百家号判重。
>
> ⚠️ **防重机制**：脚本自动扫描 `outputs/*_raw.md` 中已有标题，避免推送相同标题文章；同时记录已选题号，选题池全部用完前不重复。无需手动干预。
...
- 目标关键词不能出现在结尾总结段
- 目标关键词首次出现在第 3-5 段之间
- **全文目标关键词 4-7次**，分散在不同段落，禁止集中
- **不能有独立的「证书规划」章节**——关键词素材自然融入各段落
- 无表格、无小圆点、无AI痕迹词
- 营销词（必须/一定/唯一/最好/强烈推荐/推荐/报名/课程/培训/购买/咨询）每篇不超过2次
- 能力建设部分是全文最大篇幅
- 尾缀已加上：> 【扫码"CDA认证"小程序】...（示例尾缀，配置区可替换）
...
**⚠️ 尾缀定义（重要）：**
- 尾缀 = 引导文字 + 二维码图（默认示例：扫码了解CDA数据分析师认证…… + @8.jpg，配置区可替换）
...
3. 目标关键词 ≥ 8 次（R002）- REJECT 级别
4. 独立证书模块/集中堆砌（R014）- -15
5. 培训导向（R013）- -50
6. 目标关键词位置错误（R003/R004）- -15
7. 结构缺失（R010）- -20/-10
8. 营销词超标（R006）- -5/次
9. 目标关键词次数不足 ≤2（R001）- -12
10. AI 痕迹词（R007）- -5
11. 话术重复（R005）- -5
