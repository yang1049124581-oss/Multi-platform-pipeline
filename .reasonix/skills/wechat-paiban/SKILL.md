...
| `@cda-random` | 底部随机CDA图（从 4.png / 6.png / 7.jpg 选一张，compose 自动追加到底部） |
| `@8.jpg` | 底部固定CDA二维码（compose 自动追加在文字后） |
...
**输出**：`_composed.md`（自动插入所有图片 + CDA 底部，**不修改源文件**）

| 步骤 | 操作 |
|------|------|
| ① 清除旧 CDA | 去掉正文中已有 `@cda-random`、`@8.jpg`、扫码文字等，避免重复 |
| ② 清除旧图片 | 去掉已有 `@1.png` / `@2.png` / `@3..png`，防重复插入 |
| ③ 插入正文三图 | `@1.png` → 第1个 `##` 章节末尾；`@2.png` → 第3个 `##` 章节末尾；`@3..png` → 第5个 `##` 章节末尾 |
| ④ 追加 CDA 底部 | `@cda-random`（随机4/6/7） → 引导文字 → `@8.jpg` |
...
> **禁止**将两篇文章合并到一个 .md 文件后再拆分——CDA 底部会丢失。
